/* 
	...
	DATE:			24/01/2015
	DESCRIPTION:	Multiple interacting "Bounds & bounce" oscillators with
					drag & momentum transfer.
					Based on db.bounce~
	...
	TODO:
		...
		3) Lanes
				...
				- add 2nd dimension for balls
				- add control to set 2nd dimension (sig rate for timing?)
...
Friction acts on balls
	means we need to store rate separately from input

	Input >= current vel sets vel (optional slew up?)
	input < current vel, rate decays under friction


Optional momentum transfer
	off: impacts at different rates are as in drag~
	on: impacts transfer momentum between balls
		Maybe divide balls into 2 categories
			Free: Actual vel > control vel
			Driven: control vel >= actual vel

			Driven balls are not themselves affected by momentum
				but do give momentum to slower balls which they strike
			Free balls both give (lose) and receive (gain) momentum

Possible 2d version, with "lanes"
	Balls also have movement in 2nd dimension -can set rates
	2nd dimension divided into "lanes" - contiguous regions
	Balls only collide when in the same "lane"

Output modes
	-Path of balls (as drag~)
	-Velocity of impacts - held until next impact
		+ive if going up, -ive if going down - so always square wave about 0
...
void	drag_dsp64(t_drag *x, t_object *dsp64, short *count, double samplerate, long maxvectorsize, long flags);
void	drag_assist(t_drag *x, void *b, long msg, long arg, char *dst);	
...
double	 do_shaping (t_drag *x, t_double lo, t_double hi) ;
double	 drag_alimit(double a, double width, double t);
...
int C74_EXPORT main (void)
{
	...
	post("db.drag~ by Daniel Bennett skjolbrot@gmail.com");
	...
	while(samples--){
		...
		for(x->curr_v=0; x->curr_v < x->voice_count; x->curr_v++){			
			...
			if(x->symm_conn[v]) { // WITH SYMM SIGNALS CONNECTED
				...
				grad = drag_alimit(1/symm_l, width, t);
			} else {	// WITHOUT SYMM SIGNALS CONNECTED
				grad = drag_alimit(x->grad[v], width, t);
			}
			...
			// apply dcblock if on
			if(x->dcblock_on[v]){
				*(out[v]) = drag_dcblock(*(out[v]),&x->dc_prev_in[v], &x->dc_prev_out[v], (t_double) DCBLOCK_GAIN);
			} 
		}
